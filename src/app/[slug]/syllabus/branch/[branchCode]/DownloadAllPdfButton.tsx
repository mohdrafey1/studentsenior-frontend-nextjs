...
interface DownloadAllPdfButtonProps {
...
export default function DownloadAllPdfButton({
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            doc.text(`${branchName} - Complete Syllabus`, pageWidth / 2, 25, {
                align: 'center',
            });
            ...
            const fileName = `${branchName.replace(/\s+/g, '_')}_Complete_Syllabus.pdf`;
            doc.save(fileName);
        } catch (error) {
    ...
    return (
        <button
            ...
            className='w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all font-medium shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105'
        >
            <Download className='w-5 h-5' />
            {isGenerating ? 'Generating PDF...' : 'Download All as PDF'}
        </button>
    );
}
