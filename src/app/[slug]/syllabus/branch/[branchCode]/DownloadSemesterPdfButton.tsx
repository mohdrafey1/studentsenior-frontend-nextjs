...
interface DownloadSemesterPdfButtonProps {
    syllabusList: ISyllabus[];
    branchName: string;
    semester: number;
}

export default function DownloadSemesterPdfButton({
    syllabusList,
    branchName,
    semester,
}: DownloadSemesterPdfButtonProps) {
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            doc.text(
                `${branchName} - Semester ${semester} Syllabus`,
                pageWidth / 2,
                25,
                { align: 'center' },
            );
            ...
            syllabusList.forEach((syllabus, syllabusIndex) => {
                // Start each subject on a new page (except the first one)
                if (syllabusIndex > 0) {
                    doc.addPage();
                    addWatermark();
                    yPosition = margin;
            ...
            // Save the PDF
            const fileName = `${branchName.replace(/\s+/g, '_')}_Semester_${semester}_Syllabus.pdf`;
            doc.save(fileName);
        } catch (error) {
    ...
    return (
        <button
            ...
            className='w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-lg font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg'
        >
            <Download className='w-5 h-5' />
            {isGenerating
                ? 'Generating PDF...'
                : `Download Semester ${semester} PDF`}
        </button>
    );
}
